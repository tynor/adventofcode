...
struct valmap
{
    struct valmap *child[4];
    struct str key;
    int32_t value;
};

int32_t *getval(struct valmap **m, struct str key, struct arena *a)
{
    for (uint64_t h = str_hash(key); *m; h <<= 2) {
        if (str_eq(key, (*m)->key)) {
            return &(*m)->value;
    ...
    *m = arena_push_t(a, struct valmap);
    (*m)->key = key;
    int32_t *val = &(*m)->value;
    *val = -1;
    return val;
        ...
        struct arena *a)
{
    ...
    return eval_wire2(i.wire, m, vm, a);
}
        ...
        struct arena *a)
{
    ...
    assert(w);
    uint16_t r = 0;
    uint16_t v0 = eval_input(w->inputs[0], m, vm, a);
    if (w->type == WIRE_VALUE) {
        r = v0;
    } else if (w->type == WIRE_NOT) {
        r = ~v0;
    } else {
        uint16_t v1 = eval_input(w->inputs[1], m, vm, a);
        switch (w->type) {
        case WIRE_AND:
            r = v0 & v1;
            break;
...
uint16_t eval_wire(struct str name, struct wiremap *m, struct arena a)
{
    return eval_wire2(name, m, 0, &a);
}
...
int main(void)
{
    ...
    uint16_t av = eval_wire(S_LIT("a"), m, a);
    struct wire *w = getwire(&m, S_LIT("b"), 0);
    *w = (struct wire){
        .type = WIRE_VALUE,
        .inputs = {{.value=(int32_t)av}},
    };
    uint16_t v = eval_wire(S_LIT("a"), m, a);
    printf("%d\n", v);
}
