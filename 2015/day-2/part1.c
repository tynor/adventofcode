...
ssize_t dims_paper(struct dims ds);
...
ssize_t dims_paper(struct dims ds)
{
    ssize_t s0 = ds.d0 * ds.d1;
    ssize_t s1 = ds.d0 * ds.d2;
    ssize_t s2 = ds.d1 * ds.d2;
    ssize_t smin = MIN(s0, MIN(s1, s2));
    return s0*2 + s1*2 + s2*2 + smin;
}
